...
void rtc_init_hse(void)
{
    rt_kprintf("GET RTC FLAG TIMEOUT , use HSE\n\r");
    /* Enable PWR and BKP clocks */
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR | RCC_APB1Periph_BKP, ENABLE);
...
{
    if(kt_hw_config.rtc_config==HW_RTC_NONE)
    {
        return;
    }
    int c = RTC_GetCounter();
    rtc.type    = RT_Device_Class_RTC;

    /* Check if the Power On Reset flag is set */
    if(kt_hw_config.rtc_config==HW_RTC_HS)
    {
        rtc_init_hse();
    }
    else
    {
        if((BKP_ReadBackupRegister(BKP_DR1) != 0xA5A5) || (c == 0))
        {
            rt_kprintf("rtc is not configured,%x ,clock=%d \n", BKP_ReadBackupRegister(BKP_DR1), c);
