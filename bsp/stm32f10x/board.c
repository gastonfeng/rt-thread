...
#elif defined(STM32_USEBOOT)  /* VECT_TAB_FLASH  */
    /* Set the Vector Table base location at 0x08000000 */
    NVIC_SetVectorTable(NVIC_VectTab_FLASH, 0x0000C000);
#else
...
//      if(!(RCC->APB2ENR & 1))      //设置jtag模式
//          rt_kprintf("\n\r ERROR:RCC->APB2ENR=0x%x \n\r", RCC->APB2ENR);


    AFIO->MAPR&=0XF8FFFFFF; //清除MAPR的[26:24]
    AFIO->MAPR|=temp;       //设置jtag模式
//      AFIO->MAPR = 0x4000000;     //设置jtag模式
//      if(AFIO->MAPR != 0x4000000)     //设置jtag模式
//          rt_kprintf("\n\r ERROR:AFIO->MAPR=0x%x \n\r", AFIO->MAPR);
    //  else break;
    //      }
    rt_kprintf("\n\r JTAG MODE=0x%x \n\r", mode);
}
